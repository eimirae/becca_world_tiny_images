...
class World(BaseWorld):
    ...
    def __init__(self, lifespan=None, test=False, visualize_period= 10 ** 4, print_all_features = True, fov_horz_span = 32, fov_vert_span = 32, name = 'tiny_images_world'):
        ...
        self.filenames = ['data_batch_1', 'data_batch_2', 'data_batch_3', 'data_batch_4', 'data_batch_5']
        self.filenames = map(os.path.join, ['becca_world_tiny_images']*5, ['data'] * 5, self.filenames)
    ...
    def step(self, action): 
        print "timestep", self.timestep, " / ", self.VISUALIZE_PERIOD
        self.timestep += 1
        
        go = True
        while go:
            if self.data_index >= len(self.data_dict['labels']):
                if self.TEST:
                    self.surprise_log.close()
                    print 'End of test reached'
                    tools.report_roc(self.ground_truth_filename, self.surprise_log_filename, self.name)
                    sys.exit()
                else:
                    self.initialize_image_set()
            else:
                print self.data_dict['labels'][self.data_index]
                if self.data_dict['labels'][self.data_index] == self.imageset:
                    go = False
                    pixel_data = self.data_dict['data'][self.data_index].astype('float') / 256
                    self.data_index += 1
                else:
                    self.data_index += 1
        ...
        #center_surround_pixels = wtools.center_surround(self.intensity_image, self.fov_span)
        #unsplit_sensors = center_surround_pixels.ravel()
        #self.sensors = np.concatenate((np.maximum(unsplit_sensors, 0), np.abs(np.minimum(unsplit_sensors, 0))))
        self.sensors = pixel_data
        reward = 0
